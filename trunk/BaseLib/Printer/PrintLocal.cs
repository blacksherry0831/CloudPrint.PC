...
namespace BaseLib
{
   public static class PrintLocal
    {
...
*/
        ...
        public static bool IsPrinterOnLocal(String fPrinterName)
        {
           // String DriveName = "";
            if (!String.IsNullOrEmpty(fPrinterName)&&_printerTable.Equals(fPrinterName))
            {
                // ManagementObject mo = (ManagementObject)_printerTable[fPrinterName];
                // DriveName = mo["DriverName"].ToString();
                return true;
...
*/
        ...
        * 
        */
        public static String  GetPCLPrinter()
        {
            String printer_t = GetPrinter("pcl 6");
            if (String.IsNullOrEmpty(printer_t)) {
                printer_t = GetPrinter("pcl");
            }
            return printer_t;
       ...
       * 
       */
        public static String GetUsefulPrinter(String printerName,String printerDriveName)
        {


            if (IsPrinterOnLocal(printerName))
            {
                //本地有此打印机
                return printerName;
            }
                ...
                // 本地找不到这个打印机
                return GetPrinterByDrivenName(printerDriveName);
            }
        ...
        * 
        */
        public static String GetPrinter(String id)
        {
            foreach (String fPrinterName in PrinterSettings.InstalledPrinters)
            {
                String DriverName = GetPrinterDrivenName(fPrinterName);
                if (DriverName.ToLower().Contains(id.ToLower())){
                    return fPrinterName;
                }                
            }
            return null;
