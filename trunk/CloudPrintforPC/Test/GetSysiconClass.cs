...
namespace CloudPrintforPC
{
    ...
    class GetSysicon
    {
        ...
        public static void GetSysIcon(string path, ImageList imglist, bool flag)
        {
            ...
            if (flag)
            {
                ...
                else
                {
                    ...
                    SHGetFileInfo(dir.FullName, (uint)0x80, ref shfi,
                        (uint)Marshal.SizeOf(shfi),//Marshal.SizeOf返回对象的非托管大小
                           (uint)(0x100 | 0x400));//取得icon和typename
                    imglist.Images.Add(dir.FullName, (Icon)Icon.FromHandle(shfi.hIcon).Clone());
            ...
            else
            {
                ...
                {
                    SHGetFileInfo(file.FullName, (uint)0x80, ref shfi,
                        (uint)Marshal.SizeOf(shfi), (uint)(0x100 | 0x400));
                    imglist.Images.Add(file.FullName, (Icon)Icon.FromHandle(shfi.hIcon).Clone());
                    DestroyIcon(shfi.hIcon);
                }
            }
        }
        ...
        public static void GetSysIcon(string path, ImageList imglist)
        {
#if false

            if (path.Contains(".")){
                GetSysIcon(path,imglist,false);
            }
            else { 
                GetSysIcon(path,imglist,true);
            }
#else
            if (File.Exists(path))
            {
        ...
        public static string  GetSysKey(string path)
        {
#if false
            if (path.Contains("."))
            {
